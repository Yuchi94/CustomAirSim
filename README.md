...
This is the final project for the Deep Reinforcement Learning course at CMU. Deep reinforcement learning often allows machines to learn policies that maximize a certain reward in a way that may not seem natural to humans and yet works phenomenally well. Through deep reinforcement learning, we wanted to see if multiple autonomous vehicles can learn the rules to navigate an unstructured 4-way intersection efficiently and safely. To do so, we leverage the realism and customizability of AirSim.

AirSim is a realistic drone and automobile simulator from Microsoft, built on top of the Unreal Engine.  At the time of this project, AirSim did not support multiple agents for automobiles. This project modifies the AirSim plugin to enable control of multiple agents and uses Deep Reinforcement Learning (DDPG) to train multiple agents to simultaneously navigate a 4-way intersection. States into the network are the past 5 frames of the front view and the action space is continuous for both the steering and acceleration. Checkout the learnt policy below below!
